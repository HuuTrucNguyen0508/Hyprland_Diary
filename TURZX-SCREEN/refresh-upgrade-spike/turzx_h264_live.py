...
def soften_layout_gaps(layout: Image.Image, palette: Palette) -> Image.Image:
    """Hide the mid-screen rule H.264 makes of the top-row card bottoms + GAP.

    Four cards share the same bottom y, so their outlines read as one black bar
    across the glass (through sparklines + logo). Paint over that seam.
    """
    ...
    # Header → cards
    y0 = MARGIN + HEADER_H
    draw.rectangle((MARGIN_LEFT, y0, WIDTH - MARGIN, y0 + GAP), fill=palette.panel)
    # Cover top-row card bottom borders (2px) + GAP down to lower panels.
    cards_h = 300
    cards_top = MARGIN + HEADER_H + GAP
    cards_bottom = cards_top + cards_h
    draw.rectangle(
        (MARGIN_LEFT, cards_bottom - 2, WIDTH - MARGIN, cards_bottom + GAP),
        fill=palette.panel,
    )
    return out
