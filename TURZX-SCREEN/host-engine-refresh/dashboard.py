...
def main() -> int:
    ...
    try:
        while not stop:
            ...
            # Speedtest: push full frames as fast as USB allows (~5 fps ceiling).
            # Scheme burst: mild busy rate. Idle: 1 Hz + dirty skip.
            if speed_visible:
            ...
            dirty = logical_dirty_key(renderer.palette, speed_state, stats)
            # Never dirty-skip during speedtest — needles need every frame.
            if (
                dirty == last_dirty
